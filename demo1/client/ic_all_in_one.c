...
#define RANGE(min,max) ((int)random() % ((max) - (min) +1)) + (min)
	...
	ic_influx_database("localhost", 8086, "exec");
	ic_influx_userpw("nigel", "secret");

/* Intitalise */

  /* get the local machine hostname */
	if( gethostname(myhostname, sizeof(myhostname)) == -1) {	error("gethostname() failed");	}
...
/* Wait until we need to capture the data again */
	...
	sleep(1); 	/* Typically, this would be 60 seconds */
    }
exit(0);
}
